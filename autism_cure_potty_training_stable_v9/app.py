...
@active
def potty(pid):
    # Potty Training uses its own fresh table so old/partial databases can never
    # break this page. Existing patient records remain untouched.
    p=patient(pid)
    if not p:
    ...
    try:
        ...
        rows=c.execute("SELECT id,patient_id,log_date,kind,result,prompt,reward,hygiene,note,created_at FROM potty_training_entries WHERE patient_id=? ORDER BY id DESC LIMIT 15",(pid,)).fetchall()
    except sqlite3.Error:
        # Never expose a 500 page for this module. If the DB has a temporary
        # problem, the page still opens and the caregiver can try again.
        rows=[]
    finally:
        c.close()
