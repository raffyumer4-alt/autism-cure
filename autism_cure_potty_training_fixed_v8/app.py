...
def potty(pid):
 ...
 if not p: return redirect(url_for('dashboard'))
 c=db()
 # Robust migration: older deployments may have a potty_logs table with only
 # patient_id/log_date (or another partial schema). Never assume its columns.
 c.execute('CREATE TABLE IF NOT EXISTS potty_logs(id INTEGER PRIMARY KEY AUTOINCREMENT, patient_id INTEGER, log_date TEXT)')
 cols={r['name'] for r in c.execute('PRAGMA table_info(potty_logs)').fetchall()}
 required=[('patient_id','INTEGER'),('log_date','TEXT'),('kind','TEXT'),('result','TEXT'),('prompt','TEXT'),('reward','TEXT'),('hygiene','TEXT'),('note','TEXT'),('created_at','TEXT')]
 for name, definition in required:
  if name not in cols:
   c.execute(f'ALTER TABLE potty_logs ADD COLUMN {name} {definition}')
 c.commit()
 if request.method=='POST':
  ...
  c.execute('INSERT INTO potty_logs(patient_id,log_date,kind,result,prompt,reward,hygiene,note,created_at) VALUES(?,?,?,?,?,?,?,?,?)',
 ...
 rows=c.execute('SELECT id,patient_id,log_date,kind,result,prompt,reward,hygiene,note,created_at FROM potty_logs WHERE patient_id=? ORDER BY id DESC LIMIT 15',(pid,)).fetchall()
 c.close()
