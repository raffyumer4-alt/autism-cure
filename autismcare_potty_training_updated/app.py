...
def potty_training(pid):
    ...
    if request.method == 'POST':
        ...
        c.execute('INSERT INTO potty_logs(patient_id,log_date,event,success,prompt_level,note,created_at) VALUES(?,?,?,?,?,?,?)', (
            ...
            request.form.get('prompt_level',''), request.form.get('note',''), datetime.now().isoformat()))
        c.commit()
        rows = c.execute('SELECT * FROM potty_logs WHERE patient_id=? ORDER BY id DESC LIMIT 20', (pid,)).fetchall()
        c.close()
