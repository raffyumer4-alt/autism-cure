...
@active
def report(pid):
 p=patient(pid); c=db(); logs=c.execute("SELECT a.domain,a.title,l.score,l.status,l.logged_at FROM activity_logs l JOIN activities a ON a.id=l.activity_id WHERE l.patient_id=? AND date(l.logged_at)>=date('now','-6 day') ORDER BY l.id DESC",(pid,)).fetchall(); wb=c.execute("SELECT * FROM wellbeing_logs WHERE patient_id=? AND date(log_date)>=date('now','-6 day')",(pid,)).fetchall(); ex=c.execute("SELECT COALESCE(SUM(minutes),0) m FROM exercise_logs WHERE patient_id=? AND date(log_date)>=date('now','-6 day')",(pid,)).fetchone()['m']; goals=c.execute('SELECT * FROM goals WHERE patient_id=?',(pid,)).fetchall(); c.close(); avg=round(sum(x['score'] for x in logs)/len(logs),1) if logs else 0; return render_template('report.html',p=p,logs=logs,wb=wb,exercise=ex,goals=goals,avg=avg)
