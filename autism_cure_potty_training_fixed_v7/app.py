...
def potty(pid):
 ...
 if not p: return redirect(url_for('dashboard'))
 c=db()
 # Backward-compatible migration for databases created before Potty Training v6.
 c.execute('CREATE TABLE IF NOT EXISTS potty_logs(id INTEGER PRIMARY KEY AUTOINCREMENT,patient_id INTEGER,log_date TEXT,kind TEXT,result TEXT,prompt TEXT,reward TEXT,hygiene TEXT,note TEXT,created_at TEXT)')
 cols={r['name'] for r in c.execute('PRAGMA table_info(potty_logs)').fetchall()}
 for name, definition in [('kind','TEXT'),('result','TEXT'),('prompt','TEXT'),('reward','TEXT'),('hygiene','TEXT'),('note','TEXT'),('created_at','TEXT')]:
  if name not in cols: c.execute(f'ALTER TABLE potty_logs ADD COLUMN {name} {definition}')
 if request.method=='POST':
  kind=request.form.get('kind','trip')
  c.execute('INSERT INTO potty_logs(patient_id,log_date,kind,result,prompt,reward,hygiene,note,created_at) VALUES(?,?,?,?,?,?,?,?,?)',(pid,date.today().isoformat(),kind,request.form.get('result'),request.form.get('prompt'),request.form.get('reward'),request.form.get('hygiene'),request.form.get('note'),datetime.now().isoformat()))
  c.commit(); flash('Potty practice save ho gayi.','success')
 rows=c.execute('SELECT * FROM potty_logs WHERE patient_id=? ORDER BY id DESC LIMIT 15',(pid,)).fetchall(); c.close()
